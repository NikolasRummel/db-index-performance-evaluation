...
Another point which should be mentioned is that the choice of the `SyncInterval` being `500` in the buffer manager (`Pager` component) from @pager was made without a empirical justification. A more thorough analysis of the impact of different synchronisation intervals on write throughput would have been beneficial to better compare the real performance differences between B-Trees and LSM-Trees. However the main differences between the two structures are so significant that this would not change the overall conclusion too much.
