...
The implementation of the B-Tree and B+-Tree was done from scratch, while for the LSM-Tree, the Pebble library was used. The benchmarks were designed to evaluate the performance of each index structure under different workloads. As theoretically expected, the B+-Tree outperformed the standard B-Tree in range query scenarios, while the LSM-Tree won in write-heavy workloads. The results clearly demonstrate the architectural differences between the structures and therefore the importance on choosing the right index structure or in general the right #gls("DBMS") for the right use case.
...
In addition, the B-Trees are not fully implemented. A `delete()` method is missing, and a lot is not optimized or left out for simplicity and time constraints. For example, also normal B-Trees have a pointer to the next leaf node, which returns `nil` to easily implement one generic Tree implementation for both B-Trees and B+-Trees. In addition, in both B-Tree implementations, the nodes can have internal fragmentation. Updates with larger values do not reclaim the space of the previous version, leading to fragmentation, therefore wasted space und ultimately to worse performance. 
