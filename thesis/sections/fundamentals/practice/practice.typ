...
`PostgreSQL` is an open-source relational #gls("DBMS") that uses a special B-Tree proposed by Lehman, Philip L. and Yao, S. Bing @postgres_btree_paper, which optimizes the performance of concurrent transactions. Like in `MySQL`, the B-Tree is organized as a B+-tree, where the leaf nodes contain the actual data records, and the internal nodes contain pointers to the leaf nodes. 

`SQLite` was started in 2000 and has since become the most widely deployed database engine in the world @sqlite_most_deployed. It is written in `C` and was designed to be integrated in embedded systems like mobile applications, therefore being very lightweight @sqlite_general and easy to use. 
