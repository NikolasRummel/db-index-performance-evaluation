...
`MySQL` was first released in 1995 and has become a widely used #gls("DBMS") with over 1 billion installations @mysql_release. In the default storage engine for `MySQL`, `InnoDB`, data is organized into  B-Trees @mysql_physical_structure. However, the data is stored in the leaf nodes, so `InnoDB`'s B-Tree is technically a B+-Tree. To optimize performance, `InnoDB` uses 16KB pages by default. For sequential inserts, it keeps pages 15/16 full to allow for future growth. For random inserts, the fill factor varies between 50% and 93.75%. If a page's data drops below the 50%  due to deletions, `InnoDB` automatically merges neighboring pages to prevent fragmentation and optimize the on-disk footprint @mysql_physical_structure.
