...
#figure(
  caption: [Balanced version of the previous tree structure in @unbalanced-tree],
  cetz.canvas({
    import cetz.draw: *
...
1. Like in normal search trees, we have an alternating sequence of keys and pointers in each node. However, now a B-Tree stores the values on the disk, so each entry needs also a record pointer to the actual data record. This results in a #gls("Node") structure of $<P_1, (K_1, "RP"_1), P_2, (K_2, "RP"_2), ..., P_(q-1), (K_(q-1), "RP"_(q-1)), P_q>$, where each $"RP"_i$ is the record pointer to the actual data record and $q <= p$.
2. Like in @search-trees, the keys in each #gls("Node") are ordered such that $K_1 < K_2 < dots < K_(q-1)$ holds.
3. All search key values $X$ within a subtree pointed to by $P_i$ are bounded by the keys of the parent node. This ensures that subtrees contain only values from its parent's key space. More formally, for all values $X$ in the subtree rooted at #gls("Node") $P_i$, the following conditions hold @elmasri2016[p. 619]:
  - For $1 < i < q$: $K_(i-1) < X < K_i$
  - For $i = 1$: $X < K_i$
  - For $i = q$: $K_(i-1) < X$
...
5. All #gls("Internal Node", plural: true) (except the root and leaves) have at least $ceil(p/2)$ tree pointers to ensure some kind of density to avoid wasting space. However, the root #gls("Node") has at least two tree pointers if it is not a #gls("Leaf Node") meaning it is the only #gls("Node") in the tree.
...
==== Lookup 
A lookup operation in a B-Tree is similar to a normal search tree like in @unbalanced-tree @kleppmann[p. 80]. We start at the root #gls("Node") and compare the search key with the keys in the node. If we find a match, we return the corresponding record pointer to the disk and read from there the corresponding value. If not, we follow the appropriate pointer to the child #gls("Node") based on the key comparison. We repeat this process until the key is found or a NULL pointer in a #gls("Leaf Node") is reached, meaning the search key is not present in the tree.
...
The advantage now is that all #gls("Leaf Node", plural: true) are linked together in a list, allowing for efficient range queries by following the pointer to the next #gls("Leaf Node") after finding the start key in the #gls("Leaf Node").
...
transaction to maintain an index such as this in real time, increasing the total system cost up to fifty percent @lsm_original[p. 351]. Secondly, after a page split a B-Tree, some space in those pages is wasted, which leads to fragmentation @kleppmann[p. 84]. In addition, those B-Trees are not crash-safe since they update in place, meaning that in case of a crash while a merge or split is happening, the tree could be left in an inconsistent state @lsm_original[p. 351]. To mitigate this problem, a #gls("WAL") can be used, which would lead to a lot of #gls("writeamplification"), since we would have to write the log entry and then write the actual data to the disk, which would double the write cost @kleppmann[p. 82].


=== LSM-Trees (Log-Structured Merge Trees)
Log-Structured Merge Trees (LSM-Trees) are a type of index structure designed for high write throughput @lsm_original[p. 351] and was originally proposed by O'Neil et al. in 1996 @lsm_original. The reason for the design of a new index structure was that standard disk-based index structures such as the B-tree have some drawbacks mentioned in @drawbacks-btree, which are especially problematic for write-heavy workloads. LSM-Trees are designed to optimize write performance by batching writes together and writing them sequentially to disk, which minimizes the number of disk I/O operations required for each write and thus significantly improves write performance @lsm_original[p. 351]. 

==== LSM-Tree Structure according to O'Neil et al. <lsm_oneil>
In LSM-Trees, writes are batched together for index updates, meaning that the index on disk is not immediately updated for each write operation. Instead, updates are written to an in-memory structure and periodically merged with the on-disk index @lsm_original[p. 355]. 
This is done using a hierarchy of components (also called trees):

- *$C_0$ Component:* This is the in-memory component where all new writes are initially stored. It could use a 2-3 Tree or AVL Tree, since it does not need to insist on disk page size constraints @lsm_original[p. 356]. 2-3 or AVL Trees are another type of balanced search tree, which will not be explained in detail here, but they also have a logarithmic time complexity @intro_algorithms[p. 358] @intro_algorithms[p. 502]. All new writes are first written to this component. Since it is in-memory, it allows for very fast write operations. This implies two things: First, the data needs to be written to the disk ($C_1$ Component) at some point and second, the data is not safely stored in case of a crash @lsm_original[p. 355]. 
...
The main idea of O'Neil et al. is still the same, but since `Google` published `Bigtable`, the common term for the $C_0$ component is #gls("Memtable"), while the on-disk components collections of so-called #gls("SSTable", plural: true) @lsm_survey[p. 2]@kleppmann[p. 78].  #gls("SSTable", plural: true) are representations of #gls("Memtable", plural: true) which are written sequentially in a sorted order, making the LSM-Tree fast while flushing @lsm_survey[p. 2] data to the disk. In addition, next to a #gls("SSTable"), a small index is maintained, that is used to provide lookup for the particular #gls("SSTable") @lsm_survey[p. 2]. Also, all LSM-Trees use #gls("Bloom") to optimize read performance, which is suggested in the original paper by O'Neil et al. @lsm_original[p. 381]. To ensure data durability, the #gls("Memtable") is often backed by a #gls("WAL"), which is written to disk before the #gls("Memtable") is updated. This way, in case of a crash, the system can recover the data from the log and rebuild the #gls("Memtable") @lsm_survey[p. 2].

A complete architecture of an LSM-Tree now looks like the following in @lsm_fig.
...
While an LSM-Tree with the rolling merge process ensures efficient sequential writes, it comes with some drawbacks. First of all, since it is an append-only structure, it leads to increased storage requirements due to the presence of multiple on-disk components and tombstone markers for deletions. Secondly, this leads to a high #gls("writeamplification") since data exists in multiple components and needs to be merged multiple times. Lastly, the merge process takes some resources and therefore decreases the overall performance of the tree, which will be discussed in @evaluation.