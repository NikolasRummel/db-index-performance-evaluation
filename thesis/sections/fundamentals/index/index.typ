...
As mentoned, there are different data structures which can be used as index structures in a database system. In the following, two index structures, #gls("B-Tree", plural: true) and #gls("LSM-Tree", plural: true), will be explained in more detail.

=== Search Tree-Based Index Structures <search-trees>
#let p = $p$
In order to understand the most common data structure for DBMS, the #gls("B+-Tree") @elmasri2016 [p. 618], we will start with standard search trees. 
...
=== #gls("B-Tree", plural: true) <btree>
To ensure that a search tree stays balanced, we can use a #gls("B-Tree"). They where first described by Bayer and McCreight in 1972 @btree_original and are widely used in database systems both releational and non-relational @kleppmann[p. 80].
#gls("B-Tree", plural: true) are search trees with some additional constraints to ensure that the tree remains balanced @elmasri2016[p. 619], thus maintaining a $O(log n)$ time complexity for search, insert, and delete operations. Another reason in favor of #gls("B-Tree", plural: true) is that the #gls("Node") size can be fixed to a database #gls("Page") size. This alignment ensures that adding or removing a #gls("Node") corresponds exactly to the allocation or deallocation of a single database #gls("Page"), allowing the buffer manager to efficiently manage the pages. 

In addition, #gls("B-Tree", plural: true) were designed because the complete index structure does not fit in memory, so the tree is stored on disk @btree_original[p. 173]. This means that only a part of the tree is in memory at any given time, and the rest is stored on disk. To manage this, each #gls("B-Tree") #gls("Node") contains a list of keys, pointers to child #gls("Node", plural: true) and record pointers to the actual data records on disk. 
...
==== Lookup 
A lookup operation in a #gls("B-Tree") is similar to a normal search tree like in @unbalanced-tree @kleppmann[p. 80]. We start at the root #gls("Node") and compare the search key with the keys in the node. If we find a match, we return the corresponding record pointer to the disc and read from there the according value. If not, we follow the appropriate pointer to the child #gls("Node") based on the key comparison. We repeat this process until we either find the key or reach a NULL pointer in a #gls("Leaf Node"), meaning the search key is not present in the tree.

==== Insertion 
Insertion might seem trivial at first, if there is some space in a node, insert the new key in the correct position to maintain the order. However as described Garcia-Molina et al. @dbsystems_complete [pp. 640-641], if the #gls("Node") is full, we need to split the #gls("Node") into two #gls("Node", plural: true) and promote the middle key to the parent #gls("Node") to maintain the #gls("B-Tree") properties. This process may propagate up to the root node, potentially increasing the height of the tree @btree_original[p. 178].

==== Deletion 
Again, deletion is most likely to be more complex than just removing the key from the node. First, it performes a lookup to find the key to be deleted @dbsystems_complete[p. 642] @btree_original[p. 190]. If the key is found in a #gls("Leaf Node"), it can be removed directly. However, if the key is in an #gls("Internal Node"), we again need to maintain the #gls("B-Tree") properties described in @dbsystems_complete[p. 643] @btree_original[pp 180-182].

==== Range Query
Rangies Querys are a common operation in database systems, where we want to retrieve all keys within a specific range, for instance all titles between 'A' and 'D' @dbsystems_complete[p. 639]. For this example, we would do a lookup for the start key 'A' and then traverse the #gls("Leaf Node", plural: true) sequentially until we reach the end key 'D' with depth first in order walk. However, this will traverse all #gls("Node", plural: true) in the range, which can be inefficient if the range is large. #gls("B+-Tree", plural: true) address this issue with #gls("Leaf Node") chaining @, which will be explained in the next section.

=== #gls("B+-Tree", plural: true) <b-plus>
#gls("B+-Tree", plural: true) are a variant of #gls("B-Tree", plural: true), fixing the problem of inefficient range queries in normal #gls("B-Tree", plural: true). For this reason, #gls("B+-Tree", plural: true) are the most commonly used index structure in database systems @elmasri2016[p. 622]. 
The main difference between #gls("B-Tree", plural: true) and #gls("B+-Tree", plural: true) is that in #gls("B+-Tree", plural: true), all data pointers are stored in the leafe nodes, resulting that #gls("Internal Node", plural: true) only store keys and pointers to child nodes, but no record pointers to the actual data records @elmasri2016[p. 622].
...
Now, for a lookup, we follow the same logic like in a normal #gls("B-Tree"), but will wend in a leaf page in order to get the reccord pointer to the actual data record on the disk. For a range query, we can now follow the pointer to the next #gls("Leaf Node") after finding the start key in the #gls("Leaf Node"), which allows for efficient range queries by traversing the linked list of #gls("Leaf Node", plural: true). 


==== Drawbacks of #gls("B-Tree", plural: true) <drawbacks-btree>
However, the #gls("B+-Tree") (and #gls("B-Tree") as well) still is not a perfect solution for every possible scenario and they have some drawbacks. First, they are not optimized for write-heavy workloads, since each write operation requires multiple disk I/O operations to maintain the index structure on disk @lsm_original[p. 351]. This will effectively double the I/O cost of the
transaction to maintain an index such as this in real time, increasing the total system cost up to fifty percent @lsm_original[p. 351]. Secondly, after a page split a #gls("B-Tree"), some space in those pages is wasted, which leads to fragmentation @kleppmann[p. 84]. In addition, those #gls("B-Tree", plural: true) are not crash-safe since they update in place, meaning that in case of a crash while a merge or split is happening, the tree could be left in an inconsistent state @lsm_original[p. 351]. To mitigate this problem, a #gls("WAL") can be used, which would lead do a lot of #gls("writeamplification"), since we would have to write the log entry and then write the actual data to the disk, which would double the write cost. @kleppmann[p. 82]


=== #gls("LSM-Tree", plural: true) (Log-Structured Merge Trees)
Log-Structured Merge Trees (#gls("LSM-Tree", plural: true)) are a type of index structure designed for high write throughput @lsm_original[p. 351] and was originally proposed by O'Neil et al. in 1996 @lsm_original. The reason for the design of a new index structure was that the standard disk-based index structures such as the B-tree have some those drawbacks mentioned in @drawbacks-btree, which are especially problematic for write-heavy workloads. #gls("LSM-Tree", plural: true) are designed to optimize write performance by batching writes together and writing them sequentially to disk, which minimizes the number of disk I/O operations required for each write and thus significantly improves write performance @lsm_original[p. 351]. 

==== #gls("LSM-Tree") Structure according to O'Neil et al. <lsm_oneil>
The fundamental concept of an #gls("LSM-Tree") is based to batch writes together for index updates, meaning not immediately updating the index on disk for each write operation, but instead writing to an in-memory structure and periodically merging it with the on-disk index @lsm_original[p. 355]. 
...
*Insertion* in a #gls("LSM-Tree") works by first writing the new key-value pair to the in-memory $C_0$ component. When the $C_0$ component becomes full, it is merged with the on-disk $C_1$ component, and the process repeats.

*Updating* of a key-value pair in a #gls("LSM-Tree") is basically an insertion. However, the old value still exists in the #gls("LSM-Tree"). This is because the new value is written to the $C_0$ component, while the old value still exists in the $C_1$ component. 

*Deletion* of a key-value pair in a #gls("LSM-Tree") is also an insertion, but instead of writing the new value, we write a special tombstone value to the $C_0$ component. This tombstone value indicates that the key has been deleted, and when the $C_0$ component is merged with the $C_1$ component, the old value will be removed from the on-disk component.

A *Lookup* in a #gls("LSM-Tree") now works by starting in the in memory $C_0$ component and if the key is not found there, we continue searching in the on-disk components $C_1, C_2, ...$ starting from the lowest $C_k$ component until the key is found, a tombstone appears or all components have been searched. This can lead to inefficient read performance, since we might have to search through multiple on-disk components, which is a major drawback of #gls("LSM-Tree", plural: true). To mitigate this problem, #gls("LSM-Tree", plural: true) often use #gls("Bloom"), a data structure to quickly check if a key is likely to be present in an on-disk component before performing a more expensive search @kleppmann[p. 79].  

As mentioned, the $C_0$ Component is periodically merged into the $C_1$ Component, which O'Neil et al. call a "rolling merge" @lsm_original[p. 355]. The rough idea is to merge the $C_0$ and $C_1$ components together, by using a merge sort-like process, where we read the sorted keys from both components and write them into a new on-disk component $C_1$ while maintaining the sorted order. Since this is done in a sequential manner, there is no need for seek time and rotational latency of discs, which allows for very efficient write operations in comparison to #gls("B-Tree", plural: true) @lsm_original[p. 358]. This process is then repeated for the other on-disk components $C_k$ with $k in N$ as well, where we merge the smaller, higher-level component $C_n$ with the larger, lower-level $C_(n+1)$ to produce a new, optimized $C_(n+1)$ component @lsm_original[p. 355].
...
==== #gls("LSM-Tree") Structure in Practice
The main idea of O'Neil et al. is still the same, but since Google published `Bigtable`, the common term for the $C_0$ component is #gls("Memtable"), while the on-disk components collections of so called #gls("SSTable", plural: true) @lsm_survey[p. 2]@kleppmann[p. 78].  #gls("SSTable", plural: true) are #gls("Memtable", plural: true) which are written sequentially in a sorted order, making the #gls("LSM-Tree") fast in flushing @lsm_survey[p. 2]. In addition, next to a #gls("SSTable"), a small index is maintained, that is used to provide lookup for the particular #gls("SSTable") @lsm_survey[p. 2]. Also, all #gls("LSM-Tree", plural: true) use #gls("Bloom") to optimize read performance, which is suggested in the original paper by O'Neil et al. @lsm_original[p. 381]. To ensure data durability, the #gls("Memtable") is often backed by a #gls("WAL"), which is written to disk before the #gls("Memtable") is updated. This way, in case of a crash, the system can recover the data from the log and rebuild the #gls("Memtable") @lsm_survey[p. 2].

A complete architecture of an #gls("LSM-Tree") know looks like the following in @lsm_fig.
...
==== Drawbacks of #gls("LSM-Tree", plural: true)
While #gls("LSM-Tree") withthe rolling merge process ensures efficient sequential writes, it comes with some drawbacks. First of all, since its a append-only structure, it leads to increased storage requirements due to the presence of multiple on-disk components and tombstone markers for deletions. Secondly, this leads to a high #gls("writeamplification") since data exists in multiple components and needs to be merged multiple times. Lastly, the merge process takes some ressources and therefore decrese the overall performance of the tree, which will be discussed in @evaluation.