...
To understand #gls("Index", plural: true) structures and their implementation in a #gls("DBMS"), one must first understand the basic components and architecture of a #gls("DBMS"). A #gls("DBMS") is not just a simple program, but rather a complex system that consists of several components that work together to manage and manipulate data efficiently. While the definition by Elmasri and Navathe provides an idea on functional requirements of a #gls("DBMS"), its architecture shows the mechanisms that enable it to fulfill these requirements. The architectural components of a #gls("DBMS") can be broadly categorized into serveral components illustrated in @dbms_fig.
...
Users and database administrators interact with the #gls("DBMS") though specific #gls("DDL") and #gls("DML") statements. The `DDL Compiler` processes data definition statements, which are used to define the database schema, including tables, #gls("Index", plural: true), and other database objects. For end users, the `Query Processor` is responsible for data manipulation and retrieval. 
