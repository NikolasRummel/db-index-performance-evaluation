...
Depending on the use case, this data needs to be processed in different ways. For instance, in an #gls("OLTP") system, data needs to be inserted, updated and queried very fast to provide a good user experience. In contrast, in an online analytical processing #gls("OLAP") system, large amounts of data are analyzed to gain insights and generate reports for instance.

Traditionally, database have used #gls("B+-Tree")s as index structures to optimize data access. However, with the increasing data sizes and changing workloads, new index structures like #gls("LSM-Tree")s have been developed to address the challenges of write-intensive workloads and more and more variations of #gls("B-Tree")s are being used to optimize read performance.

As a consequence, the choice of the right index structure is crucial for the performance of a database system. Different index structures have different strengths and weaknesses, and the optimal choice depends on the specific workload and use case. Therefore, #gls("DBMS") developers must select the appropriate index for their specific software system, while software engineers need to understand the underlying principles of their chosen #gls("DBMS") to select the right platform and effectively optimize their application design for performance

== Objectives and Research Questions <research_questions>
In order to make a decision on which index structure to use, this project aims to explore and compare the performance of different index structures. This will be done by implementing a benchmark that evaluates the performance of #gls("B-Tree")s, #gls("B+-Tree")s and #gls("LSM-Tree")s under different workloads and data sizes. For the implementation, a #gls("B-Tree") and a #gls("B+-Tree") will be implemented from scratch, while for the #gls("LSM-Tree"), an existing implementation will be used in order to save time and focus on the evaluation. As inspiration the rough design of the #gls("B-Tree") and #gls("B+-Tree") implementations will be based on `SQLite` concept of storing all data in one file. 
...
RQ1: How do #gls("B-Tree")s, #gls("B+-Tree")s, and #gls("LSM-Tree")s compare in terms of query speed?

RQ2: How significant is the performance gap between #gls("B-Tree")s and #gls("B+-Tree")s during range queries?
